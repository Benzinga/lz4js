...
exports.uncompressBound = function uncompressBound(src) {
    ...
    var useContentSum = (descriptor & fdContentChksum) != 0;
    ...
    // Get content size
    if (useContentSize) {
        return util.readU64(buf, sIndex);
    }
...
exports.uncompressBlock = function uncompressBlock(src, dst, sIndex, sLength, dIndex) {
    ...
    while (sIndex < sEnd) {
        ...
        if (literalCount > 0) {
            ...
            // Copy literals
            for (var n = sIndex + literalCount; sIndex < n;)
        ...
        // Copy match.
        for (var i = dIndex - mOffset, n = i + mLength; i < n;) {
            dst[dIndex++] = dst[i++] | 0;
        }
    }
    ...
    if (sLength >= minLength) {
        ...
        while (sIndex + minMatch < sEnd - searchLimit) {
            ...
            if (literalCount >= runMask) {
                ...
                for (var n = literalCount - runMask; n >= 0xff; n -= 0xff)
                    dst[dIndex++] = 0xff;
                dst[dIndex++] = n;
            } else {
            ...
            // Write match length.
            if (mLength >= mlMask) {
                for (var n = mLength - mlMask; n >= 0xff; n -= 0xff)
                    dst[dIndex++] = 0xff;
                dst[dIndex++] = n;
            }
